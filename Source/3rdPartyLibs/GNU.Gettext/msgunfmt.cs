    ...
    private void Dump (GettextResourceSet catalog) {
      ...
      // Now the other messages.
      {
        ...
        foreach (DictionaryEntry entry in catalog)
          if (!"".Equals(entry.Key)) {
            object value = catalog.GetObject(entry.Key.ToString());
            string key_plural =
              (plural != null && value is string[] ? plural[entry.Key.ToString()] as string : null);
            DumpMessage(entry.Key.ToString(), key_plural, value);
          }
      }
    }
