...
namespace ORTS
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            Text = String.Format(version.Length > 0 ? "{0} {1}" : "{0}", Application.ProductName, version);
#if DEBUG
            Text = Text + " (debug)";
#endif
        ...
        void buttonOptions_Click(object sender, EventArgs e)
        {
            ...
            using (var form = new OptionsForm(Settings, false))
            {
        ...
        void LoadFolderList()
        {
            ...
            FolderLoader = new Task<List<Folder>>(this, () => Folder.GetFolders(Settings).OrderBy(f => f.Name).ToList(), (folders) =>
            {
                ...
                {
                    using (var form = new OptionsForm(Settings, true))
                    {
                        switch (form.ShowDialog(this))
                        {
                            case DialogResult.OK:
